	...
	public class DotGraphBuilder_Tests
	{
        ...
        public void JustNodesWithAttributes()
        {
            var dot = DotGraphBuilder.DirectedGraph("NoEdges")
                ...
                .Build();
            var should = @"digraph NoEdges { 
        n1 [color=black; shape=box]; n2 [fontsize=12; label=""node №2""; shape=ellipse] }"; // для отладки (убрать)
            AssertAreSame(should, dot);
        }
		...
		{
            var arg1 = NormalizeString(expectedDot); // для отладки (убрать)
            var arg2 = NormalizeString(actualDot); // для отладки (убрать)
