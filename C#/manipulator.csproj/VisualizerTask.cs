...
namespace Manipulation
{
	public static class VisualizerTask
	{
		...
		{
            PointF pos = GetShoulderPos(form);

            X = ConvertWindowToMath(new PointF(e.X, e.Y), pos).X;
            Y = ConvertWindowToMath(new PointF(e.X, e.Y), pos).Y;
		...
		public static void DrawManipulator(Graphics graphics, PointF shoulderPos)
		{
            ...
            PointF elbow = AnglesToCoordinatesTask.GetJointPositions(Shoulder, Elbow, Wrist)[0];
            elbow = ConvertMathToWindow(elbow, new PointF(0, 0));
            PointF wrist = AnglesToCoordinatesTask.GetJointPositions(Shoulder, Elbow, Wrist)[1];
            wrist = ConvertMathToWindow(wrist, new PointF(0, 0));
            PointF palm = AnglesToCoordinatesTask.GetJointPositions(Shoulder, Elbow, Wrist)[2];
            palm = ConvertMathToWindow(palm, new PointF(0, 0));

            graphics.DrawLine(ManipulatorPen, new PointF(0,0), elbow);
            graphics.FillEllipse(JointBrush, elbow.X, elbow.Y, (float)1, (float)1);

            graphics.DrawLine(ManipulatorPen, elbow, wrist);
            graphics.FillEllipse(JointBrush, wrist.X, wrist.Y, (float)1, (float)1);

            graphics.DrawLine(ManipulatorPen, wrist, palm);
            graphics.FillEllipse(JointBrush, palm.X, palm.Y, (float)1, (float)1);
		}
