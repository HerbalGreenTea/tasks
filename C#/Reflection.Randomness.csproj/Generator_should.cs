...
namespace Reflection.Randomness
{
	...
	public class Generator_should
	{
        ...
        private void AssertPropertyFilledWithDistribution(double actualValue, IContinousDistribution distribution)
        {
            ...
            var sequenceStart = new[] { distribution.Generate(random), distribution.Generate(random), 
                distribution.Generate(random), distribution.Generate(random), distribution.Generate(random) };
            random = new Random(seed);
            random.NextDouble();
            var sequence = sequenceStart.Concat(new[] { distribution.Generate(random), distribution.Generate(random), 
                distribution.Generate(random), distribution.Generate(random), distribution.Generate(random) });
